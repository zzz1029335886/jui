...
typedef ImageItemRender = Widget Function(BuildContext context, io.File file,
    Size size, Function(io.File file) onRemove);
...
class _PictureSelectionState extends State<PictureSelection> {
  ...
  Widget build(BuildContext context) {
    return StaggeredGrid.count(
      ...
      children: [
        ...List.generate(_renderImages.length, (index) {
          return InkWell(
              onTap: () {
                ImagePickers.previewImages(
                    _renderImages.map((e) => e.path).toList(), index);
              },
              child: _renderImageItem(_renderImages[index]));
        }).toList(),
  ...
  Future<void> _shoot() async {
    ...
    ImagePickers.openCamera().then((Media? media) {
      ...
      if (media != null && media.path != null) {
        _renderImages.add(io.File(media.path!));
        _refreshUi();
      }
    });
  }
  ...
  Future<void> _photoAlbumSelect() async {
    ...
    if (widget.multipleChoice) {
      selectCount = widget.maxCount - _renderImages.length;
    }
    ...
    if (listImagePaths.isNotEmpty) {
      var fs = List<io.File>.from(listImagePaths.map((e) => io.File(e.path!)))
          .toList();
